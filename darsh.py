...
def doctor_appointments():
    ...
    def fetch_doctor_slots(city, specialty):
        # Mocked appointment slots - replace with live API data
        return {
        ...
        for doctor, times in slots.items():
            ...
            for t in times:
                if st.button(f"Book {doctor} at {t}"):
                    st.success(f"Appointment booked with {doctor} at {t}. Confirmation will be sent.")
...
# Real-time ambulance driver contact (simulated)

def fetch_ambulance_driver_contact(location):
    # Simulated API response for ambulance driver info
    example_response = {
        "driver_name": "Rajesh Kumar",
        "driver_phone": "88222 88222",
        "ambulance_id": "AMB-1234",
        "eta_minutes": 7,
        "vehicle_type": "Basic Life Support Ambulance"
    }
...
def emergency_support():
    ...
    if st.button("Request Ambulance Driver Contact"):
        if not location.strip():
            ...
            return
        st.info(f"Searching ambulance driver for location: {location}...")
        time.sleep(1)  # Simulate API call delay
        driver_info = fetch_ambulance_driver_contact(location)
        st.success(f"Ambulance Driver Found: {driver_info['driver_name']}")
        st.write(f"Phone: {driver_info['driver_phone']}")
        st.write(f"Ambulance ID: {driver_info['ambulance_id']}")
        st.write(f"Estimated arrival: {driver_info['eta_minutes']} minutes")
        st.write(f"Ambulance Type: {driver_info['vehicle_type']}")
        st.info("Please call the driver directly to coordinate your emergency.")
